...
class Benchmark(abc.ABC):
    ...
    def get_task_init_states(self, i):
        ...
        print(init_states.shape, init_states_path)
        return init_states
...
@register_benchmark
class LIFT_AND_PLACE(Benchmark):
    def __init__(self, task_order_index=0):
        super().__init__(task_order_index=task_order_index)
        self.name = "lift_and_place"
        self._make_benchmark()

    def _make_benchmark(self):
        tasks = list(task_maps[self.name].values())
        # 为4个任务创建所有可能的顺序列表
        lift_and_place_orders = [
            [0, 1, 2, 3],  # 原始顺序
        ...
        # 确保task_order_index不超过可用的顺序数量
        valid_order_index = self.task_order_index % len(lift_and_place_orders)
        print(f"[info] using lift_and_place task orders {lift_and_place_orders[valid_order_index]}")
        self.tasks = [tasks[i] for i in lift_and_place_orders[valid_order_index]]
        self.n_tasks = len(self.tasks)
